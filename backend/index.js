...
const races = Object.entries(namedays).map(([date, names], index) => ({
  id: index + 1,
  race: formatDate(date),
  names,
}));
...
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/races') {
    res.writeHead(200);
    res.end(JSON.stringify(races.map(r => ({ id: r.id, race: r.race }))));
  } else if (parsed.pathname === '/winner') {
    const id = parseInt(parsed.query.id);
    const entry = races.find(r => r.id === id);
    if (entry) {
      res.writeHead(200);
      res.end(JSON.stringify({ winner: entry.names.join(', ') }));
    } else {
      res.writeHead(404);
      res.end(JSON.stringify({ error: 'Nincs adat' }));
    }
  } else {
    res.writeHead(404);
